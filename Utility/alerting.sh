...
REAL_BLOCK=$(curl -s "$PUBLIC_TRUSTED_RPC/status" | jq '.result.sync_info.latest_block_height' | xargs )
...
if [[ $REAL_BLOCK -eq 0 ]]; then
    MSG="$ip can't connect to $PUBLIC_TRUSTED_RPC"
